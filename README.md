...
- **Tracker** absorbs Compute run UI in the prototype: artifacts **grouped by SAP section** (collapsible), full-width title/path bands, plain Type/Status labels (fewer pills), row multi-select, **Run selected (R/SAS)**, job stubs, tags, import from study `programs/`; Send to QC also freezes share ACLs (cross-link Files). Page intro/sub copy uses full content width.
